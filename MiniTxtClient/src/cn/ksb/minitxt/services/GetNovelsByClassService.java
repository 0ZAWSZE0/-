...
public class GetNovelsByClassService extends BaseServiceImpl<Classification> {
	...
	public Service<? extends Serializable> execute() {
		DataTransfer<Classification> dto = new DataTransfer<Classification>();
		dto.setKey(Constants.COMMAND_GETNOVELS);
		dto.setData(getInputData());
		
		DefaultCommunitatorImpl<Classification, Novel[]> comm =
				new DefaultCommunitatorImpl<Classification, Novel[]>();
		...
		while (true) {
			...
			if(choice == -1) {
				...
				default:
					...
					download = ServiceFactory.getService(Constants.COMMAND_DOWMLOAD);
